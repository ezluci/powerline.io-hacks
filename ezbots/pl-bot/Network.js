...
export class Network {
   constructor(KT2) {
      ...
      var cO = this;
      ((async () => {
         cO.atbairohcv = '00000000000000000000000000000000', cO.atbairohcvr = !0x0, cO.isConnecting && !cO.hasConnection && cO.getServerAndConnect();
      })());
   }
   ...
   connect() {
      ...
      try {
         ...
         // this._pv = 396795;
         let string = 'b' + this._pv.toString();
         if (KT.bProxyString) {
            string += '+' + KT.bProxyString;
         }
         this.webSocket = new WebSocket('ws://localhost:44444', string);
      } catch (cN) {
   ...
   disconnect() {
      var cV = iF;
      KT.network.isConnecting = !0x1, KT.network.directed && (!KT.network.inIframe() && (0), KT.network.directed = !0x1), KT.network.roomID = 0x0, KT.network.webSocket && KT.network.webSocket.close();
   }
   ...
   CreateEntity(cV, cq) {
      ...
      switch (cV) {
         ...
         cO = new EntityPlayer(KT);
         break;
         case this.ENTITY_ITEM:
         cd.etEXt(cq, this.SUB_ENTITY_ITEM_FOOD) && (cO = new EntityFood(KT));
         break;
         default:
   ...
   updateEntities(cV) {
      for (var cq = iF, cY = {
         ...
         switch (cd += 0x1, cf) {
            ...
            } else cY.AcNnE(slog, cY.omEOS(cY.ulLzV, cO), 0x0);
            break;
            default:
   ...
   processConfig(cV) {
      ...
      function cl(cj, cN, cE) {
         var cP = cj.getUint32(cN, !0x0);
         cD[0x0] = cP ^ cE;
         return cI[0x0];
      }
   ...
   processMessage(packet) {
      ...
      if (opcode == this.OPCODES.SC_PONG) {
         ...
         console.log('CAPTCHA!! You\'re cooked lol');
         /*$("#statsPanel").hide();
         $("#basePanel").show();
         $("#topGui").hide();
   ...
   connectionClosed() {
      for (var cV = iF, cq = {
         ...
         switch (cY[cd++]) {
            ...
            continue;
            case '12':
            KT.network.kickedInactive ? 0 : KT.focus && (console.log(cq.sjPhf), KT.network.connectVar = cq.VHyfb(setTimeout, KT.network.getServerAndConnect, cq.LDuzX(0x3e8, cq.SITGt(cO, 0x3e8))));
            continue;
            case '13':
   ...
   sendHello() {
      ...
      var bytes = [//133,6,    
         ...
         //82,53,137,196,45,26,166,23,104,34,119,167,153,237,87,106
         //27,230, 87, 60,251,163, 36, 27,55, 67,240,251,230, 85,192, 60
      ];
      // console.log(bytes);
      for (let i = 11; i < 27; ++i) {
   ...
   sendTalk(cV) {
      if ("IlGpy" !== "XDbwH") {
         var cq = iF,
         cY = {};
         cY.oeLvB = "0|3|2|4|1", cY.hVgop = function(cJ, cf) {
            return cJ ^ cf;
         };
         for (var cd = cY, cO = cd.oeLvB.split('|'), cD = 0x0;;) {
            switch (cO[cD++]) {
               case '0':
               ...
               continue;
               case '2':
               cl.setUint8(0x0, cd.hVgop(KT.network.OPCODES.TALK, KT.network.opcode_cycle_s));
               continue;
               case '3':
               var cl = new DataView(cI);
               continue;
         ...
         var cf = this.curLength - cY.startLength;
         return cf /= 0x3e8, (cf += 2.5) > 0x8 && (cf = 0x8), cf * (0x5 - 0x4 * this.snakeScale);
      }
   }
