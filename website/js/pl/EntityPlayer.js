class EntityPlayer {
   constructor() {
      ...
      this.highlight = false; // ill use this
   ...
   update(cV) {
      ...
      if (this.highlight) {
         cV.strokeStyle = "rgba(255, 83, 83, 0.28)";
         cV.lineWidth = 20,
         this.drawTail(this.renderedPoints, cV);
      }
      ...
      if (this.extraSpeed > 0xa) {
         ...
         cV.shadowBlur = 0, cV.strokeStyle = "hsl(" + this.hue + ", 100%, 90%)",
         cV.lineWidth = (cd - 0.8) * this.snakeScale;
         this.drawTail(this.renderedPoints, cV),
      ...
      var Dv = a0C;
      cV.fillStyle = "rgba(255, 255, 255, 0.9)", cV.font = `Bold ${11 / Math.max(KT.camera.zoom, 0.5)}px
         'proxima-nova-1','proxima-nova-2', Arial`, cV.textBaseline = "hanging";
      var cq = this.nick.substring(0x0, 0x10);
      /*this.drawSpeed &&*/ (cq += ' (' + this.ping + ')' + (this.mobile ? ' M' : ''));
      var cY = cV.measureText(cq).width;
      if (this.highlight) {
         cV.fillStyle = "rgba(28, 231, 75, 0.9)"
         cV.shadowColor = "rgba(255, 255, 255, 1)";
         cV.shadowBlur = 3;
      }
      cV.fillText(cq, -cY / 0x2, 25);
         // cv.shadowBlur = 3;
      ...
      if (cY) {
         ...
         this.hue = cV.getUint16(cq, !0x0), cq += 0x2, this.mobile = 0x1 == cV.getUint8(cq), cq += 0x1, this.colorStyle = "hsl(" + this.hue + ", 100%, 50%)", this.eyesStyle = "hsl(" + this.hue + ", 100%, 20%)", this.brightStyle = "hsl(" + this.hue + ", 100%, 90%)", this.lightStyle = "hsl(" + this.hue + ", 100%, 70%)", this.headDotCanvas = KT.resources.frames.head_dot.renderTintedFrame("#FFFFFF"), this.renderedGlowFrame = KT.resources.frames.glow.renderTintedFrame(this.colorStyle), this.renderedGlowHardFrame = KT.resources.frames.glow_hard.renderTintedFrame(this.brightStyle), this.lightningFrames.push(KT.resources.frames.lightning1.renderTintedFrame(this.colorStyle)), this.lightningFrames.push(KT.resources.frames.lightning2.renderTintedFrame(this.colorStyle)), this.lightningFrames.push(KT.resources.frames.lightning3.renderTintedFrame(this.colorStyle)), this.sparkFrames.push(KT.resources.frames.spark0.renderTintedFrame(this.lightStyle)), this.sparkFrames.push(KT.resources.frames.spark1.renderTintedFrame(this.lightStyle)), this.sparkFrames.push(KT.resources.frames.spark2.renderTintedFrame(this.lightStyle));
   ...
   ["addTestTurnPoint"](cV, cq) {
      ...
      if (this.id == KT.localPlayerID && KT.killCount + 0x1 >= KT.KILLS_TO_FOLLOW && (KT.lastKillerID = this.killedByID, KT.cameraSmooth = 0x1e), this.killedByID == KT.localPlayerID && this.id == KT.kingID && this.id != KT.localPlayerID && KT.isInGame && KT.hud.addSpecialMessage("YOU KILLED THE KING!", 0x1e), KT.focus && this.killReason != KT.KILL_REASON_LEFT_SCREEN) {
         this.beingDeleted = !0x0, this.id == KT.localPlayerID && KT.camera.shake(0x7 + 0.13 * this.extraSpeed);
